// telemetry source object for the Aircraft_44
...
function Aircraft_44() {

	
	// Initialize working Parameters and Object

	// read the keys from dictionary
	let rawDict = fs.readFileSync('../openmct/example/Aircraft_44/Aircraft_44dictionary.json')
			...
			try{ // store in history
				...
				if(this.continousLogging){					
					...
					this.asyncStringyify(message).then(function(write) {//write is the value of the resolved promise in asyncStringify
						fs.appendFile(__dirname + '/saved_logs/Aircraft_44_'+this.FileTimestamp+'_rawMessage.json', write, (err) => {
							if (err) {
								console.log(err);
							} 
						}) 
	...
	server.on('error', (err) => {
		console.log(`Aircraft_44 UDP server error:\n${err.stack}`);
		try{
    ...
    console.log("Aircraft_44 initialized!");
};


// to update every time new data comes in
Aircraft_44.prototype.generateRealtimeTelemetry = function () {
...
Aircraft_44.prototype.generateIntervalTelemetry = function () {
...
Aircraft_44.prototype.notify = function (point) {
...
Aircraft_44.prototype.listen = function (listener) {
...
Aircraft_44.prototype.SetFileTimestamp = function () {
...
Aircraft_44.prototype.asyncStringyify = function (obj) {
...
Aircraft_44.prototype.command = function (command) {
	...
	if(command === ':saveHistory'){
		...
		this.asyncStringyify(this.history).then(function(write) {//write is the value of the resolved promise in asyncStringify
			fs.writeFile(__dirname + '/saved_logs/Aircraft_44_'+this.FileTimestamp+'_History.json', write, (err) => {
				if (err) {
					console.log(err);
				} else {
	...
	// 			fs.writeFile(__dirname + '/saved_logs/Aircraft_44_'+this.FileTimestam+'.json', write, (err) => {
	// 				if (err) {
	// 					throw err;
	// 				}
