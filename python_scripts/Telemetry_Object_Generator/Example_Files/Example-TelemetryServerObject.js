// telemetry source object for the EXAMPLE
			...
			try{ // store in history
				...
				if(this.continousLogging){					
					...
					this.asyncStringyify(message).then(function(write) {//write is the value of the resolved promise in asyncStringify
						fs.appendFile(__dirname + '/saved_logs/EXAMPLE_'+this.FileTimestamp+'_rawMessage.json', write, (err) => {
							if (err) {
								console.log(err);
							} 
						}) 
...
EXAMPLE.prototype.generateRealtimeTelemetry = function () {
...
EXAMPLE.prototype.generateIntervalTelemetry = function () {
...
EXAMPLE.prototype.SetFileTimestamp = function () {
...
EXAMPLE.prototype.asyncStringyify = function (obj) {
...
EXAMPLE.prototype.command = function (command) {
	...
	if(command === ':saveHistory'){
		...
		this.asyncStringyify(this.history).then(function(write) {//write is the value of the resolved promise in asyncStringify
			fs.writeFile(__dirname + '/saved_logs/EXAMPLE_'+this.FileTimestamp+'_History.json', write, (err) => {
				if (err) {
					console.log(err);
				} else {
	...
	// 			fs.writeFile(__dirname + '/saved_logs/EXAMPLE_'+this.FileTimestam+'.json', write, (err) => {
	// 				if (err) {
	// 					throw err;
	// 				}
